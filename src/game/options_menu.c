...
static const u8 bindStr[][32] = {
    ...
    { TEXT_BIND_SPEEDKICK }, { TEXT_BIND_QUICKTURN }, { TEXT_OPT_DEADZONE }, { TEXT_OPT_RUMBLE },
};
...
#define DEF_OPT_TOGGLE(lbl, bv) { .type = OPT_TOGGLE, .label = lbl, .bval = bv }
...
#define DEF_OPT_SUBMENU(lbl, nm) { .type = OPT_SUBMENU, .label = lbl, .nextMenu = nm }
#define DEF_OPT_BIND(lbl, uv) { .type = OPT_BIND, .label = lbl, .uval = uv }
#define DEF_OPT_BUTTON(lbl, act) { .type = OPT_BUTTON, .label = lbl, .actionFn = act }
#define DEF_SUBMENU(lbl, opt) { .label = lbl, .opts = opt, .numOpts = sizeof(opt) / sizeof(opt[0]) }
...
static struct Option optsControls[] = {
    ...
    DEF_OPT_BIND(bindStr[16], configKeySpeedkick), DEF_OPT_BIND(bindStr[17], configKeyQuickturn),
    ...
    DEF_OPT_SCROLL(bindStr[18], &configStickDeadzone, 0, 100, 1),
    DEF_OPT_SCROLL(bindStr[19], &configRumbleStrength, 0, 100, 1)
};
...
#endif
    DEF_OPT_SUBMENU(menuStr[2], &menuControls), DEF_OPT_SUBMENU(menuStr[3], &menuVideo),
    DEF_OPT_SUBMENU(menuStr[4], &menuAudio), DEF_OPT_BUTTON(menuStr[5], optmenu_act_exit),
